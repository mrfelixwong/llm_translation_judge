"""
Multi-Dimensional Judge - ORIGINAL VERSION: Technique 3 - Structured reasoning with dimensions.

This is the ORIGINAL multi-dimensional judge from commit cb6e0cd that demonstrated 
the averaging problem, causing only 25% error detection rate in real experiments.

The key issue was in lines 242-243 and 305-306 where simple averaging was used:
    dim_scores = [d["score"] for d in result["dimensional_scores"].values()]
    result["overall_score"] = round(sum(dim_scores) / len(dim_scores))

This allowed high fluency/style scores to mask critical accuracy failures.
...
HISTORICAL PERFORMANCE: 25% error detection (failed on critical factual errors)
"""
...
class MultiDimensionalJudgeOriginal(BaseJudge):
    """
    Multi-dimensional LLM judge with structured reasoning - ORIGINAL AVERAGING VERSION.
    ...
    ACTUAL RESULTS: FAILED - Only 25% error detection due to averaging problem!
    """
    
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.judge_name = "Multi-Dimensional Judge (Original)"
        self.technique = "Multi-Dimensional Reasoning (Original)"
  ...
  "strengths": ["List of notable positive aspects"]
    ...
    def _parse_dimensional_response(self, response: str) -> Dict[str, Any]:
        ...
        result = {
            ...
            "methodology": "multi_dimensional_structured_evaluation_original"
        }
        ...
        # THE ORIGINAL AVERAGING PROBLEM - LINES 242-243 from original commit
    ...
    def _fallback_dimensional_parse(self, response: str) -> Dict[str, Any]:
        ...
        result = {
            ...
            "strengths": [],
            "confidence": "medium",
            "methodology": "fallback_dimensional_parsing_original"
        }
        ...
        # THE ORIGINAL AVERAGING PROBLEM - LINES 305-306 from original commit
...
if __name__ == "__main__":
    judge = MultiDimensionalJudgeOriginal()
    
    test_cases = [
        {
            "source": "Meet in room 302 at 3:00 PM.",
            "translation": "Nos vemos en la sala 205 a las 3:00 PM.",
            "description": "Critical factual error - wrong room number"
        },
        {
            "source": "Send the email immediately.", 
            "translation": "Envía el email más tarde.",
            "description": "Semantic reversal - opposite meaning"
        },
        {
            "source": "Thank you for your assistance.",
            "translation": "Gracias por tu ayuda.",
            "description": "Good quality translation"
        }
    ]
    
    print(f"Testing {judge.judge_name}")
    print("=" * 60)
    print("DEMONSTRATING THE ORIGINAL AVERAGING PROBLEM")
    print("This version shows why 25% error detection occurred")
    print()
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        for dim, data in result["dimensional_scores"].items():
            ...
            total_for_avg += data['score']
            count_for_avg += 1
        
        if count_for_avg > 0:
            manual_avg = total_for_avg / count_for_avg
            print(f"\n❌ ORIGINAL AVERAGING: ({'+'.join([str(d['score']) for d in result['dimensional_scores'].values()])})/4 = {manual_avg:.2f} → {round(manual_avg)}/5")
        
        error_threshold = 3
        if result['overall_score'] <= error_threshold:
            print(f"✅ Error DETECTED (score ≤ {error_threshold})")
        else:
            print(f"❌ Error MISSED (score > {error_threshold})")
        
        print("-" * 50)
    
    print("\n🔍 HISTORICAL ANALYSIS:")
    print("This original version achieved only 25% error detection because")
    print("high fluency/style scores (5/5) masked critical accuracy failures (2/5 or 1/5)")
    print("Example: (2+5+3+5)/4 = 3.75 → 4/5 → ERROR MISSED!")
